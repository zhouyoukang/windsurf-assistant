...
KEY = json.loads(cur.fetchone()[0]).get('apiKey', '')
con.close()
...
MODELS = [
    ...
    ("claude-sonnet-4-6",         "Sonnet 4.6 (基准✓)"),
...
for uid, label in MODELS:
    ...
    if not cid:
        print(f"  [{label}] {uid}: no cascadeId"); continue
    ...
    if 'status: 0' not in trailer:
        print(f"  [{label}] {uid}: SEND FAIL (uid rejected or disconnect): {trailer.strip()[:50]}")
        continue
    ...
    icon = "❌" if errors else "✅"
    if errors:
        print(f"  {icon} [{label}] {uid}: {errors[0][:100]}")
    else:
        print(f"  {icon} [{label}] {uid}: OK, no errors ({len(strings)} strings)")
